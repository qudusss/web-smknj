                ...
                <div class="col">
                    <form method="GET" action="{{ route('alumni') }}">
                        <div class="input-group mb-3">
                            <input type="text" name="search" class="form-control" placeholder="Cari sesuai nama anda..."
                            ...
                            <tbody class="table-group-divider">
                                @foreach ($alumni as $a)
                                    <tr class="table-primary">
                                        <td scope="row">
                                            {{ ($alumni->currentPage() - 1) * $alumni->perPage() + $loop->iteration }}</td>
                                        <td>{{ $a->nama }}</td>
                                        <td>{{ $a->jurusan }}</td>
                                        @if ($a->status == null)
                                            <td>Belum Mengisi</td>
                                        ...
                                        @endif
                                        <td>
                                            <a href="{{ route('change_status', $a->id) }}" class="btn btn-warning"><i
